...
* * [`mergeProps(stateProps, dispatchProps, ownProps): props`] \(*Function*): If specified, it is passed the result of `mapStateToProps()`, `mapDispatchToProps()`, and the parent `props`. The plain object you return from it will be passed as props to the wrapped component. You may specify this function to select a slice of the state based on props, or to bind action creators to a particular variable from props. You may also pass a factory-style function that returns a function to be used to compute the merged props. If you omit either, `Object.assign({}, ownProps, stateProps, dispatchProps)` is used by default.
