...
hide_title: true
description: 'API > useSelector: reading values from the Redux store state with a selector function'
---
...
`useSelector` accepts a pre-typed setup via `useSelector.withTypes<RootState>()`, which lets you define the `state` type once instead of specifying it in every selector. See [Usage with TypeScript](../using-react-redux/usage-with-typescript.md) for details on defining pre-typed hooks.

## See Also

- [`useSignalSelector`](./useSignalSelector.md): a fine-grained variant of this hook that tracks which parts of the state each selector reads
- [`useDispatch`](./useDispatch.md) and [`useStore`](./useStore.md): the other core React Redux hooks
